...
export async function createTransactionAction(prevState: any, formData: FormData) {
  ...
  revalidatePath(`/dashboard/portfolios/${portfolio_id}/transactions`);
  return { success: true };
}

export async function updateTransactionAction(id: string, portfolio_id: string, prevState: any, formData: FormData) {
  const type = formData.get('type');
  const rawAmount = formData.get('amount') as string;
  const description = formData.get('description');
  let category_id = formData.get('category_id');
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/portfolios/${portfolio_id}/transactions/${id}`, {
      method: 'PUT',
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
    ...
    if (!res.ok) {
      return { error: data.error || 'Gagal mengubah transaksi' };
    }
  } catch (error) {
    return { error: 'Koneksi ke server gagal' };
  }
...
export async function deleteTransactionAction(id: string, portfolio_id: string) {
  const cookieStore = await cookies();
  const token = cookieStore.get('jwt')?.value;
  ...
  try {
    ...
    if (!res.ok) {
      return { error: data.error || 'Gagal menghapus transaksi' };
    }
  } catch (error) {
    return { error: 'Koneksi ke server gagal' };
  }
